          ...
          <main class="orders-main">
            <div class="container-fluid">
              <div class="d-flex align-items-center justify-content-between mb-3">
                <h4 class="m-0">Pending order</h4>
                <a class="btn btn-outline-secondary" href="/shipper">Dashboard</a>
              </div>

              <c:if test="${empty orders}">
                <div class="alert alert-info">No PENDING order.</div>
              </c:if>
                  ...
                  <tbody>
                    <c:forEach var="o" items="${orders}">
                      <tr>
                        ...
                        <td><span
                            class="badge ${o.status == 'PENDING' ? 'bg-warning text-dark' : (o.status == 'SHIPPING' ? 'bg-primary' : 'bg-secondary')} ">${o.status}</span>
                        </td>
                        <td class="text-end">
                          <c:choose>
                            <c:when test="${o.status == 'PENDING'}">
                              <form method="post" action="/shipper/orders/${o.id}/accept" class="d-inline">
                                <input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}" />
                                <button class="btn btn-sm btn-primary" type="submit">Accept</button>
                              </form>
                            </c:when>
                            <c:otherwise>
                              <button class="btn btn-sm btn-secondary" type="button" disabled>Accepted</button>
                            </c:otherwise>
                          </c:choose>
                        </td>
                      </tr>
